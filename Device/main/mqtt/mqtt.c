...
cJSON* mqtt_create_payload(const char* device_id, const char* msg_type, int seq) {
    ...
    // 4. t_sensor_ms - Timestamp với validation
    struct timeval tv;
    gettimeofday(&tv, NULL);
    
    bool ntp_synced = (tv.tv_sec > MIN_VALID_EPOCH);
    uint64_t t_ms;
    
    if (ntp_synced) {
        // Thời gian đã được sync với NTP
        t_ms = (uint64_t)tv.tv_sec * 1000ULL + (uint64_t)tv.tv_usec / 1000ULL;
    } else {
        // Chưa sync NTP - sử dụng relative time (ms since boot)
        // Server sẽ dùng t_pc_rx_ms thay thế
        t_ms = 0;
        ESP_LOGW(TAG, "NTP not synced! t_sensor_ms = 0, server will use t_pc_rx_ms");
    }
    
    cJSON_AddNumberToObject(root, "t_sensor_ms", (double)t_ms);
    
    // 4b. Thêm flag ntp_synced để server biết
    cJSON_AddBoolToObject(root, "ntp_synced", ntp_synced);
    
    // 4c. Thêm uptime_ms để debug (luôn có giá trị)
